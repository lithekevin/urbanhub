...
export const createEmptyDatesInSchedule = function (startDate: string, endDate: string, schedule: { [date: string]: any[] } ) {
...
export const initializeAvailableAttractions = function (cityAttractions: any, adults: number, kids: number, budget: number, isFirstCall: boolean) {
...
export const fillFirstAttractionPerDay = function (availableAttractionsToBePicked: Attraction[], schedule: { [date: string]: any[] }, date: string) {
...
export const findTheCloserAttraction = function(availableAttractionsToBePicked: Attraction[], tripCity: any, previousAttractionID: string) {
...
export const computeStartTime = function(previousAttraction: Attraction, nextAttraction: Attraction, endPreviousAttractionMinutes: number) {
    ...
    const startMinutes = Math.floor(Math.ceil((startTime % 60) / 5) * 5).toString().padStart(2, "0")
...
export const computeEndTime = function(nextAttraction: Attraction, startDate: string) {
...
export const fillAttractionInSchedule = function (schedule: { [date: string]: any[] }, date: string, availableAttractionsToBePicked: Attraction[], tripCity: any) {
...
do {
    nextAttractionIndex = findTheCloserAttraction(availableAttractionsToBePicked, tripCity, previousAttractionID);
    nextAttraction = (availableAttractionsToBePicked.splice(nextAttractionIndex, 1))[0];
...
tripAttraction.startDate = computeStartTime(tripCity.attractions.find((a: Attraction) => (a.id === previousAttraction.id)), nextAttraction, previousAttractionEndHourMinutes);
...
export const fillSchedule = function (schedule: { [date: string]: any[] }, tripCity: any, adults: number, kids: number, budget: number) {

let availableAttractionsToBePicked: Attraction[] = []; // list of attractions that can be picked for the trip creation

availableAttractionsToBePicked = initializeAvailableAttractions(tripCity!.attractions, adults, kids, budget, true); // fill the list of available attractions to be picked with free attractions and attractions that fit the budget
...
for (const date in schedule) {
    ...
    if (availableAttractionsToBePicked.length === 0) { // when the list of available attractions to be picked is empty, initialize it again
        availableAttractionsToBePicked = initializeAvailableAttractions(tripCity!.attractions, adults, kids, budget, false); // fill the list of available attractions to be picked with free attractions
    }
    ...
    if (schedule[date].length === 0) { // if the schedule date is empty, add the first attraction in a random way starting from 09:00
        ...
        dayCompleted = fillAttractionInSchedule(schedule, date, availableAttractionsToBePicked, tripCity);
